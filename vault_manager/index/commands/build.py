...
class BuildCommand(Command):
    ...
    def execute(self, args: Namespace) -> None:
        ...
        scanner = FileScanner(
            ...
            max_hash_size_mb=args.max_hash_size
        )
        ...
        markdown_files = {path: info for path, info in files.items()
                         if path.endswith('.md')}
    ...
    def _extract_all_links(self, markdown_files: Dict[str, FileInfo],
                           link_extractor: LinkExtractor, vault_root: Path) -> List[Link]:
    ...
    def _populate_tags_tables(self, conn: sqlite3.Connection, files: Dict[str, FileInfo]) -> int:
        ...
        cursor.executemany('INSERT OR REPLACE INTO file_tags (tag, file_path) VALUES (?, ?)', file_tags_data)
    ...
    def _populate_metadata_table(self, conn: sqlite3.Connection, files: Dict[str, FileInfo],
                                 links: List[Link], vault_root: Path) -> None:

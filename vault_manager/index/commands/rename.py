...
class RenameCommand(Command):
    ...
    def _find_mangled_names(self) -> None:
        ...
        print("3. Run: python -m Library.index rename apply")
    ...
    def _generate_report(self, renames: List[Tuple[str, str]]) -> str:
        ...
        lines.extend([
            ...
            "3. Run: `python -m Library.index rename apply`",
            "4. Links in markdown files will be automatically updated",
            "5. After renaming, run: `python -m Library.index build` to update the database",
            "",
            "---",
            "",
            "*Generated by `python -m Library.index rename find`*"
        ])
    ...
    def _apply_renames(self, update_links: bool = True) -> None:
        ...
        if not report_file.exists():
            ...
            print("Run 'python -m Library.index rename find' first to generate the report.")
            return
        ...
        if renamed_files:
            ...
            print("\n⚠ Important: Run 'python -m Library.index build' to update the database.")
    ...
    def _update_links(self, renames: List[Tuple[str, str]]) -> None:
        ...
        conn = sqlite3.connect(db_path)
        cursor = conn.cursor()
        cursor.execute("SELECT file_path FROM files WHERE extension = '.md'")

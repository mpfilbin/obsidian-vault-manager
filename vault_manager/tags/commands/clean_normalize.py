...
class CleanNormalizeCommand(Command):
    ...
    def execute(self, args: Namespace) -> None:
        ...
        print(f"\n{'=' * 60}")
        print("Tag Case Normalization Tool")
        print(f"{'=' * 60}")
    ...
    def _process_directory(
        self, directory: Path, vault_root: Path, ctx: DryRunContext, confirm_phase: bool = True
    ) -> Counter:
        """
        ...
        for file_path in iter_markdown_files(directory, vault_root, additional_ignores):
            ...
            try:
                success, normalized_count, changes = self._normalize_file(
                    file_path, ctx.dry_run or confirm_phase
                )
                ...
                if normalized_count > 0:
                    ...
                    ctx.record_change(
                        file_path, f"Normalized {normalized_count} tag(s)", changes=changes
                    )
                    ...
                    if not confirm_phase:
                        ...
                        mode_prefix = (
                            "[DRY RUN] Would normalize in" if ctx.dry_run else "Normalized in"
                        )

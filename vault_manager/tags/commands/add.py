...
class AddCommand(Command):
    ...
    def execute(self, args: Namespace) -> None:
        ...
        if has_database:
            hierarchy = self._build_tag_hierarchy_from_database(vault_root, min_count=1)
            if hierarchy:
    ...
    def _build_tag_hierarchy_from_database(self, vault_root: Path, min_count: int = 2) -> Dict:
        """
    ...
    def _extract_frontmatter_with_tags(self, content: str) -> Tuple[Optional[str], str, Optional[List[str]]]:
        """
    ...
    def _generate_ai_tags(self, content: str, api_key: str, vault_root: Path) -> Tuple[List[str], List[str]]:
        """
        ...
        if hierarchy:
            ...
            tags_section = f"""The vault uses a hierarchical tagging system. Here is the complete tag taxonomy in JSON format:

{hierarchy_json}

Tag Format Rules:
1. Nested tags use SLASH notation: parent/child/grandchild (e.g., "software-development/design-principles/solid")
...
- Use full slash paths for precision (e.g., "software-architecture/distributed/microservices")
...
- "books" (standalone tag)"""
            ...
            tags_section = """The vault uses a comprehensive tagging system. Common high-value tags include:
- software-architecture, software-development, security, owasp, computer-science
...
Use kebab-case and prefer specific, established tags."""
        ...
        prompt = f"""Analyze this Obsidian markdown note and generate appropriate tags for it.
...
- Examples of VALID nested tags: software-development/design-principles, security/owasp/mobile

Return ONLY the tags as a comma-separated list, nothing else.
Use slash notation for nested tags from the hierarchy.

Example return format: software-architecture/distributed/microservices, design-principles/solid, security/owasp, books
...
{content}"""
    ...
    def _update_file_with_tags(self, file_path: Path, tags: List[str], dry_run: bool = False) -> bool:
        """

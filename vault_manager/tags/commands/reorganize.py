...
class ReorganizeCommand(Command):
    ...
    def execute(self, args: Namespace) -> None:
        ...
        print(f"\n{'=' * 60}")
        print("Tag Reorganization Analyzer")
        print(f"{'=' * 60}")
    ...
    def _generate_ai_suggestions(
        ...
        flat_tags_with_counts = [{"tag": tag, "count": tags_data[tag]} for tag in flat_tags]
        existing_hierarchy = {parent: children for parent, children in existing_parents.items()}

        # Create prompt
        prompt = self._create_ai_prompt(flat_tags_with_counts, existing_hierarchy, max_suggestions)
    ...
    def _create_ai_prompt(
        self, flat_tags: List[Dict], existing_hierarchy: Dict[str, List[str]], max_suggestions: int
    ) -> str:
        """Create the prompt for AI analysis."""
        return textwrap.dedent(
            f"""
            You are analyzing an Obsidian vault's tag taxonomy to suggest hierarchical
            reorganization.
            ...
            TASK:
            Analyze the flat tags and suggest which ones should be reorganized as children
            or grandchildren under existing parent tags or new parent tags. Consider:

            1. Semantic relationships (e.g., "microservices" → "architecture/microservices")
            2. Domain knowledge (software development, architecture, security, computer
               science, etc.)
            3. Existing hierarchy patterns (maintain consistency)
            4. Tag usage frequency (avoid reorganizing the most heavily used tags unless
               strongly justified)

            RULES:
            - Use Obsidian's tag/subtag syntax (e.g., "parent/child" or
    ...
    def _display_summary(
        self, suggestions: List[Tuple[str, str, str, float, str]], tags_data: Dict[str, int]
    ) -> None:
    ...
    def _generate_report(
        ...
        for tag, parent, suggested_path, confidence, reason in suggestions:
            ...
            lines.append(
                f"| `{tag}` | {count} | `{suggested_path}` | {confidence:.0%} | {reason} | {command} |"
            )
